...
migrate-up:
	@npx dbmate -s "$(DB_DIR)/schema.sql" -u $(DATABASE_URL) up

# Rollback the latest migration
migrate-down:
	@npx dbmate -s "$(DB_DIR)/schema.sql" -u $(DATABASE_URL) down
...
migrate-reset:
	@npx dbmate -s "$(DB_DIR)/schema.sql" -u DATABASE_URL=$(DATABASE_URL) drop
	@npx dbmate-s "$(DB_DIR)/schema.sql" -u DATABASE_URL=$(DATABASE_URL) up
